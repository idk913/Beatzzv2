...
import .sql_helper.google_drive_sql as helper
...
from ..utils import progress, humanbytes, time_formatter, human_to_bytes, admin_cmd
from . import CancelProcess, BOTLOG_CHATID,CMD_HELP, LOGS, G_DRIVE_CLIENT_ID, G_DRIVE_CLIENT_SECRET, G_DRIVE_DATA,G_DRIVE_FOLDER_ID,TMP_DOWNLOAD_DIRECTORY
...
@bot.on(admin_cmd(pattern=r"gdlist(?: |$)(-l \d+)?(?: |$)?(.*)?(?: |$)",
          outgoing=True))
...
@bot.on(admin_cmd(pattern="gd(?: |$)(.*)", outgoing=True))
